...
import { updateTaskValidator, uuidValidator } from "@/lib/validators/task.validators";
...
export const PATCH = asynchandler(async (request: NextRequest, context?: unknown) => {
    ...
    const validationResult = updateTaskValidator(body);
    if (!validationResult.success) {
        throw new ApiError("Validation error", 400, validationResult.errors);
    }

    const updatedTask = await updateTask(userId, id, validationResult.data);
